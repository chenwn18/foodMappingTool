...
export class DeleteMappingModal extends Component {
    ...
    modalInfo = () => {
        ...
        if (!attributeName)
            return (<div>
                <p>确认移除<span className='emphasize1'>{this.props.field}</span>领域的<span
                    className='emphasize1'>{generalFood[Path]}</span>与：</p>
                <p>标准食品<span className='emphasize2'>{standardFood[Path]}</span>的映射关系吗？</p>
            </div>);
        else if (!standardFood)
            return (<div>
                <p>确认移除<span className='emphasize1'>{generalFood[Field]}</span>领域的<span
                    className='emphasize1'>{generalFood[Path]}</span>与：</p>
    ...
    render() {
        ...
        return (
            <span>
                <a href="#" onClick={this.showModal}>删除</a>
                <Modal
                    visible={visible}
                    title={<span className='modalTitle'>映射删除确认</span>}
                    onOk={this.handleOk}
